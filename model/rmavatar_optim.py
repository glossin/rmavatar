...
class SplattingAvatarOptimizer(LossBase):
    ...
    def setup_optimizer(self, optimizer_config):
        ...
        if optimizer_config.get('optim_deformation', False):
            print(f'[SplattingAvatarOptim] optim_deformation, lr={optimizer_config.optim_deformation.lr}')
            l.append({
                'params': list(model._deformation.get_mlp_parameters()),
                'lr': optimizer_config.optim_deformation.lr,
                'name': '_deformation'
            })
            if optimizer_config.optim_deformation.get('scheduler_args', False):
                self.lr_schedulers['_deformation'] = _make_expon_lr_func(
                    optimizer_config.optim_deformation.scheduler_args)

        if optimizer_config.get('optim_grid', False):
            print(f'[SplattingAvatarOptim] optim_grid, lr={optimizer_config.optim_grid.lr}')
            l.append({
                'params': list(model._deformation.get_grid_parameters()),
                'lr': optimizer_config.optim_grid.lr,
                'name': '_grid'
            })
            if optimizer_config.optim_grid.get('scheduler_args', False):
                self.lr_schedulers['_grid'] = _make_expon_lr_func(optimizer_config.optim_grid.scheduler_args)
    ...
    def reset_opacity(self):
        model = self.gs_model
        opacities_new = model.inverse_opacity_activation(
            torch.min(model.get_opacity, torch.ones_like(model.get_opacity) * 0.01))
    ...
    def prune_points(self, mask):
        if self.gs_model.binding is not None:
            ...
            binding_to_prune = self.gs_model.binding[mask]
            counter_prune = torch.zeros_like(self.gs_model.binding_counter)
            counter_prune.scatter_add_(0, binding_to_prune, torch.ones_like(binding_to_prune, dtype=torch.int32, device="cuda"))
            mask_redundant = (self.gs_model.binding_counter - counter_prune) > 0
        ...
        if self.gs_model.binding is not None:
            ...
            self.gs_model.binding_counter.scatter_add_(0, self.gs_model.binding[mask], -torch.ones_like(self.gs_model.binding[mask], dtype=torch.int32, device="cuda"))
            self.gs_model.binding = self.gs_model.binding[valid_points_mask]
        ...
        for group in self.optimizer.param_groups:
            ...
            if stored_state is not None:
                ...
                group['params'][0] = nn.Parameter(
                    torch.cat((group['params'][0], extension_tensor), dim=dd).requires_grad_(True))
                self.optimizer.state[group['params'][0]] = stored_state
    ...
    def adaptive_density_control(self, render_pkg, iteration, cameras_extent=2.0):
        ...
        if iteration < opt.densify_until_iter:
            ...
            model.max_radii2D[visibility_filter] = torch.max(model.max_radii2D[visibility_filter],
                                                             radii[visibility_filter])
            self.add_densification_stats(viewspace_point_tensor, visibility_filter)

            if  iteration >= opt.densify_from_iter and iteration<= opt.densify_end_iter and iteration % opt.densification_interval == 0:
            ...
            if  iteration >= opt.densify_from_iter and iteration<= opt.densify_end_iter and iteration % opacity_reset_interval == 0:
    ...
    def save_checkpoint(self, model_path, iteration):
        ...
        # to cano
        self.gs_model.update_to_canomesh()

        # # save gaussian
        self.gs_model.save_ply(os.path.join(pc_dir, 'point_cloud.ply'))
        # import pickle
        # binding_path = os.path.join(pc_dir, 'binding.pkl')
        # with open(binding_path, 'wb') as binding_file:
        #     pickle.dump(self.gs_model.binding, binding_file)
        #
        # # save mesh embedding
        self.gs_model.save_embedding_json(os.path.join(pc_dir, 'embedding.json'))

        #save deformation
        self.gs_model.save_deform_weights(model_path, iteration)
