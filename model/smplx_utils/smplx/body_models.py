        ...
        **kwargs
        ...
        if data_struct is None:
            if osp.isdir(model_path):
                ...
                smpl_path = os.path.join(model_path, model_fn)#找到pkl文件
            else:
                smpl_path = model_path
            assert osp.exists(smpl_path), 'Path {} does not exist!'.format(
                smpl_path)

            with open(smpl_path, 'rb') as smpl_file:#读取pkl文件
        ...
        if vertex_ids is None:
            ...
            vertex_ids = VERTEX_IDS['smplh']#读取关键部位的索引
        ...
        self.register_buffer('J_regressor', j_regressor)# 6890 * 24  , 是从不同的人在不同的姿势的例子中学习回归矩阵，从mesh中回归出关节点

        # Pose blend shape basis: 6890 x 3 x 207, reshaped to 6890*3 x 207
        num_pose_basis = data_struct.posedirs.shape[-1]# 6890 * 207 * 3  23 x 9 =207所有207个姿势混合形状组成的矩阵
        # 207 x 20670
