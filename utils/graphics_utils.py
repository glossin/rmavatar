...
def compute_face_orientation(verts, faces, return_scale=False):
    i0 = faces[..., 0].long()
    i1 = faces[..., 1].long()
    i2 = faces[..., 2].long()  # 提取出每个三角形顶点的索引

    v0 = verts[..., i0, :]
    v1 = verts[..., i1, :]
    v2 = verts[..., i2, :]  # 提取出每个三角形顶点的坐标。

    a0 = safe_normalize(v1 - v0)  # 计算每个三角形的第一个边的单位方向向量
    a1 = safe_normalize(torch.cross(a0, v2 - v0, dim=-1))  # 计算两个向量的叉乘，得到三角面片的法线向量
    a2 = -safe_normalize(torch.cross(a1, a0, dim=-1))  # will have artifacts without negation与前两者垂直的第三边

    orientation = torch.cat([a0[..., None], a1[..., None], a2[..., None]], dim=-1)

    if return_scale:
        s0 = length(v1 - v0)  # 计算三角形的第一个边的长度
        s1 = dot(a2, (v2 - v0)).abs()  # （dot点积操作将 a2 和 (v2 - v0) 这两个向量的对应位置的元素相乘，然后将这些乘积相加,abs绝对值）
        scale = (s0 + s1) / 2
