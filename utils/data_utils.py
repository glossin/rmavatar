...
def sample_bary_on_triangles(num_faces, num_samples):
    sample_bary = torch.zeros(num_samples, 3)#创建一个大小为(10000, 3)的张量来存储采样点
    sample_bary[:, 0] = torch.rand(num_samples)#u
    sample_bary[:, 1] = torch.rand(num_samples) * (1.0 - sample_bary[:, 0])#v
    sample_bary[:, 2] = 1.0 - sample_bary[:, 0] - sample_bary[:, 1]#生成三个随机数作为每个采样点的重心坐标，并且保证它们的和不超过 1（保证这些点在三角形内）
    sample_fidxs = torch.randint(0, num_faces, size=(num_samples,))#在[0,9976)范围内生成10000个随机整数，这些整数表示了在哪个三角形上进行采样

    # shuffle bary重新排列每个采样点的重心坐标顺序，增加随机性，确保采样点在三角形表面上的分布更加均匀。
...
def retrieve_verts_barycentric(vertices, faces, fidxs, barys):
    triangle_verts = vertices[faces].float()#从vertices中提取出每个三角形的顶点坐标。

    if len(triangle_verts.shape) == 3:
        sample_verts = torch.einsum('nij,ni->nj', triangle_verts[fidxs], barys)#公式1.2
