...
def tbn(triangles):
    a, b, c = triangles.unbind(-2)#三角形的三个顶点
    n = F.normalize(torch.cross(b - a, c - a), dim=-1)#计算归一化法线
    d = b - a#边向量

    X = F.normalize(torch.cross(d, n), dim=-1)
    Y = F.normalize(torch.cross(d, X), dim=-1)
    Z = F.normalize(d, dim=-1)#Z对应切线(切线是与三角形的一条边平行的向量)，X对应双切线(双切线是平行于表面并与切线和法线垂直的向量)，n对应法线

    return torch.stack([X, Y, Z], dim=3)


def triangle2projection(triangles):
    R = tbn(triangles)#旋转矩阵
...
class PerVertQuaternion(nn.Module):
    ...
    def prepare_cano_per_vert(self, cano_verts, cano_faces):
        ...
        if self.use_numpy:
            ...
            face_areas = calc_face_areas(cano_verts, cano_faces)#三角形的面积
    ...
    def calc_per_vert_quaternion(self, mesh_verts):
        ...
        per_face_quat = self.calc_per_face_quaternion(mesh_verts)#每个面从规范到姿态的旋转四元数
        ...
        if self.use_numpy:
            ...
            per_vert_quat = F.normalize(verts_quats, eps=1e-6, dim=1)#对顶点的四元数表示进行归一化处理(公式5)
    ...
    def calc_per_face_Rt(self, mesh_verts):
        cano_verts = self.cano_verts
        cano_faces = self.cano_faces
        cano_triangles = cano_verts[cano_faces].unsqueeze(dim=0)#(1,9976,3,3)
        deform_triangles = mesh_verts[cano_faces].unsqueeze(dim=0)#变形之后的三角形的坐标
        per_face_Rt = calc_per_face_Rt(cano_triangles, deform_triangles)#形变空间中的三角面片映射到标准空间中
        return per_face_Rt
    
    def calc_per_face_quaternion(self, mesh_verts):
        per_face_Rt = self.calc_per_face_Rt(mesh_verts)#每个面从规范到姿态的旋转
        per_face_quat = rotation_matrix_to_quaternion(per_face_Rt[:, :3, :3])#每个面的旋转矩阵转换为四元数表示
        return per_face_quat#每个面从规范到姿态的旋转四元数
    ...
    def calc_face_area_change(self, mesh_verts, damping=1e-4):
        ...
        return change_ratio#面积的变化率
    
def calc_face_areas(mesh_verts, mesh_faces):
    vertices_faces = mesh_verts[mesh_faces]#（9976,3,3）网格面的顶点数据
    ...
    face_areas = faces_normals.norm(dim=-1, keepdim=True) / 2.0#求每个面的面积
    return face_areas
