...
class RodriguesModule(nn.Module):
    def forward(self, rvec):
        ...
        rvec_normalized = rvec / theta.unsqueeze(2)
        costh = torch.cos(theta)
        sinth = torch.sin(theta)
        rmtx = torch.stack((
            rvec_normalized[:, :, 0] ** 2 + (1. - rvec_normalized[:, :, 0] ** 2) * costh,
            rvec_normalized[:, :, 0] * rvec_normalized[:, :, 1] * (1. - costh) - rvec_normalized[:, :, 2] * sinth,
            rvec_normalized[:, :, 0] * rvec_normalized[:, :, 2] * (1. - costh) + rvec_normalized[:, :, 1] * sinth,

            rvec_normalized[:, :, 0] * rvec_normalized[:, :, 1] * (1. - costh) + rvec_normalized[:, :, 2] * sinth,
            rvec_normalized[:, :, 1] ** 2 + (1. - rvec_normalized[:, :, 1] ** 2) * costh,
            rvec_normalized[:, :, 1] * rvec_normalized[:, :, 2] * (1. - costh) - rvec_normalized[:, :, 0] * sinth,

            rvec_normalized[:, :, 0] * rvec_normalized[:, :, 2] * (1. - costh) - rvec_normalized[:, :, 1] * sinth,
            rvec_normalized[:, :, 1] * rvec_normalized[:, :, 2] * (1. - costh) + rvec_normalized[:, :, 0] * sinth,
            rvec_normalized[:, :, 2] ** 2 + (1. - rvec_normalized[:, :, 2] ** 2) * costh
        ), dim=2).view(N, B, 3, 3)
        return rmtx
